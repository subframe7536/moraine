...
- **UnoCSS:** First-class support through `@subf/unocss` with either `presetWind3()` or `presetWind4()` and Moraine's `presetMoraine()`.
...
To ensure that Moraine's semantic tokens participate in runtime conflict resolution (e.g. `z-10` overriding `z-floating`, or `opacity-100` overriding the engine-native `opacity-64` utility), `cn` is configured using `createCn`:
...
| `presetMoraine({ wind3: true })`                              | **Remove the option.** Use `presetMoraine()` with either UnoCSS `presetWind3()` or `presetWind4()`; Tailwind v3 remains unsupported.                                                                       |
...
keyframes block, and the `z-floating` plus the engine-native `opacity-64` utility. If any of these is absent from the emitted CSS, the fixture fails.

### 4.2. UnoCSS Consumer Configuration

The minimal UnoCSS setup registers either supported utility preset and Moraine's semantic theme, and explicitly scans the installed distribution when the consumer's build pipeline excludes `node_modules`:
...
their keyframes, and the `z-floating`/the engine-native `opacity-64` utility tokens. Fixture "success" is not compilation without error; it is the presence
...
2. Implement custom `cn` instance in `src/shared/utils.ts` using `createCn` with custom z-index `classGroups` (`z-base`..`z-floating`). Standard Tailwind `ring-3` is handled natively without custom extension.
...
2. **Required consumer setup:** Tailwind documentation and fixtures load `moraine/tailwind` and scan the installed `moraine/dist` through a path that is demonstrably relative to the fixture stylesheet. UnoCSS documentation and fixtures load `presetMoraine()` with both `presetWind3()` and `presetWind4()` and scan the installed distribution when needed. Both fixtures additionally assert that the **emitted CSS actually contains** the custom `data-*`/`aria-*` variant rules, the `animate-mo-enter`/`animate-mo-exit` utilities and their keyframes, and the `z-floating`/the engine-native `opacity-64` utility tokens — not merely that the engine ran without throwing.
