...
- [ ] try to pass `jsx: true` and remove `jsxImportSource: "solid-js/h"` to `mdx` compiler, so generated code is jsx and can be used in `solid-js` directly, instead of compiled by satteri using `solid-js/h/jsx-runtime`.
- [ ] try to use frontmatter to replace `<DocsHeader />` and `<DocsApiReference />`, so we can remove `docs/build/markdown/compile.ts` and `docs/build/markdown/compile.test.ts`, and also remove `DocsHeader` and `DocsApiReference` components, and also remove `docs/build/markdown/createDocsHastPlugin.ts` and `docs/build/markdown/createDocsCodePlugin.ts`.
