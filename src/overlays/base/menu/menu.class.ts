...
export const overlayMenuRecipeOptions = {
  base: {
    content:
      'text-popover-foreground p-1 outline-none border border-border rounded-md bg-popover flex flex-col min-w-36 shadow-md origin-(--mo-popper-content-transform-origin) z-floating data-closed:(animate-mo-exit exit-opacity-0 exit-scale-95) data-expanded:(animate-mo-enter enter-opacity-0 enter-scale-95) motion-reduce:animate-none data-[side=top]:(mb-(--mo-popper-content-overflow-padding) enter-translate-y-1 exit-translate-y-1) data-[side=right]:(ml-(--mo-popper-content-overflow-padding) -enter-translate-x-1 -exit-translate-x-1) data-[side=bottom]:(mt-(--mo-popper-content-overflow-padding) -enter-translate-y-1 -exit-translate-y-1) data-[side=left]:(mr-(--mo-popper-content-overflow-padding) enter-translate-x-1 exit-translate-x-1)',
