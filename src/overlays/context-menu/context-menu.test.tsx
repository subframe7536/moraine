...
describe('ContextMenu', () => {
  ...
  test('opens from keyboard context menu shortcut at trigger center', async () => {
    const screen = render(() => (
      <ContextMenu>
        <ContextMenu.Trigger as="button" type="button">
          Row Item
        </ContextMenu.Trigger>
        <ContextMenu.Content items={[{ label: 'Keyboard action' }, { label: 'Second action' }]} />
      </ContextMenu>
    ))
  ...
  test('exposes trigger data state while opened, closed, and disabled', async () => {
    const screen = render(() => (
      <ContextMenu disabled>
        <ContextMenu.Trigger as="div">Row Item</ContextMenu.Trigger>
        <ContextMenu.Content items={[{ label: 'Disabled action' }]} />
      </ContextMenu>
    ))
    ...
    const enabledScreen = render(() => (
      <ContextMenu>
        <ContextMenu.Trigger as="div">Enabled Row</ContextMenu.Trigger>
        <ContextMenu.Content items={[{ label: 'Open item' }]} />
      </ContextMenu>
    ))
  ...
  test('exposes disabled semantics for native and non-native triggers with caller overrides', () => {
    render(() => (
      <>
        <ContextMenu disabled>
          <ContextMenu.Trigger as="div">Disabled div</ContextMenu.Trigger>
          <ContextMenu.Content items={[]} />
        </ContextMenu>
        <ContextMenu disabled>
          <ContextMenu.Trigger as="button" type="button">
        ...
        <ContextMenu disabled>
          <ContextMenu.Trigger as="div" aria-disabled="false" tabIndex={2}>
            Overridden div
          </ContextMenu.Trigger>
          <ContextMenu.Content items={[]} />
        </ContextMenu>
  ...
  test('calls the trigger contextmenu handler for native right-clicks', async () => {
    ...
    const screen = render(() => (
      <ContextMenu onOpenChange={onOpenChange}>
        <ContextMenu.Trigger as="div" onContextMenu={onContextMenu}>
          Row Item
        </ContextMenu.Trigger>
        <ContextMenu.Content items={[]} />
      </ContextMenu>
    ...
    try {
      ...
      const screen = render(() => (
        <ContextMenu onOpenChange={onOpenChange}>
          ...
          <ContextMenu.Content items={[{ label: 'Touch action' }]} />
        </ContextMenu>
      ))
  ...
  test('does not open on left click', async () => {
    const screen = render(() => (
      <ContextMenu>
        <ContextMenu.Trigger as="div">Row Item</ContextMenu.Trigger>
        <ContextMenu.Content items={[{ label: 'Open by right click only' }]} />
      </ContextMenu>
    ))
  ...
  test('fires onOpenChange when opened and blocks opening when disabled', async () => {
    ...
    const enabledScreen = render(() => (
      <ContextMenu onOpenChange={onOpenChange}>
        <ContextMenu.Trigger as="div" data-testid="enabled-row">
          Enabled Row
        </ContextMenu.Trigger>
        <ContextMenu.Content items={[{ label: 'Enabled action' }]} />
      </ContextMenu>
    ))
    ...
    const disabledScreen = render(() => (
      <ContextMenu disabled onOpenChange={disabledOnOpenChange}>
        <ContextMenu.Trigger as="div" data-testid="disabled-row">
          Disabled Row
        </ContextMenu.Trigger>
        <ContextMenu.Content items={[{ label: 'Disabled action' }]} />
      </ContextMenu>
  ...
  test('supports controlled open state and reports right-click open attempts', async () => {
    ...
    const screen = render(() => (
      <ContextMenu open={false} onOpenChange={onOpenChange}>
        <ContextMenu.Trigger as="div">Row Item</ContextMenu.Trigger>
        <ContextMenu.Content items={[{ label: 'Controlled item' }]} />
      </ContextMenu>
    ))
  ...
  test('uses live controlled state and open handler when the long press completes', async () => {
    ...
    try {
      ...
      const screen = render(() => (
        <ContextMenu open={controlledOpen()} onOpenChange={onOpenChange()}>
          <ContextMenu.Trigger as="div">Row Item</ContextMenu.Trigger>
          <ContextMenu.Content items={[{ label: 'Touch action' }]} />
        </ContextMenu>
      ))
  ...
  test('renders item matrix, nested submenu, and content slots', async () => {
    ...
    expect(rootContent?.classList).toContain('data-[side=bottom]:-enter-translate-y-1')
  ...
  test('keeps nested submenus mounted through recursive exit motion', async () => {
    ...
    try {
      render(() => (
        <ContextMenu id="context-dismiss-order" defaultOpen>
          <ContextMenu.Trigger as="div">Row Item</ContextMenu.Trigger>
          <ContextMenu.Content
            items={[
              {
  ...
  test('renders into portal by default', async () => {
    const screen = render(() => (
      <ContextMenu>
        <ContextMenu.Trigger as="div">Row Item</ContextMenu.Trigger>
        <ContextMenu.Content items={[{ label: 'Default portal' }]} />
      </ContextMenu>
    ))
  ...
  test('does not open when context menu trigger is disabled', async () => {
    ...
    try {
      const screen = render(() => (
        <ContextMenu disabled>
          <ContextMenu.Trigger as="div">Row Item</ContextMenu.Trigger>
          <ContextMenu.Content items={[{ label: 'Disabled entry' }]} />
        </ContextMenu>
      ))
