...
Pass `virtualized`, `virtualRender`, and `scrollToItem` to render only visible entries. The context exposes the complete reactive collection, the mounted listbox through `scrollElement`, and `render(entry, index, props)` for forwarding measurement and positioning attributes to the final option row. `scrollToItem` receives the source Select item and its flattened entry index.
