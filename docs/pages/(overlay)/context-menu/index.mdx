...
`ContextMenu` has no root DOM element. Trigger opens the portaled Content, which renders groups and items. Put native surface attributes on Content. Root `classes` and `styles` set defaults for this menu; Content can override its slots, including nested submenu layers. See [Styling](/styling/overrides#override-component-styles).
