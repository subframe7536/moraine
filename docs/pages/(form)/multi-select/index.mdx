...
api:
  path: src/form/multi-select/multi-select
sidebar:
  order: 13
search:
...
│   │   └── input / placeholder (editable / non-editable)
│   ├── clear (optional)
│   └── trigger
...
Group headings and options render as sibling rows in the default listbox. The non-editable placeholder is an internal element, not a style slot.
