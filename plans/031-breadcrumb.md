# Plan 031: Expose Breadcrumb links without requiring duplicate data
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/navigation/breadcrumb 'docs/pages/(navigation)/breadcrumb' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx`. Also inspect `git status --short` and `git diff -- src/navigation/breadcrumb 'docs/pages/(navigation)/breadcrumb' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose List/Item/Link/Page/Separator/Ellipsis/Items and keep items/itemRender default rendering. Manual links need neither items nor value registration. Items renders entries/separators inside an explicit List. Preserve to ?? href precedence, native anchors and router host forwarding. Page conveys the current page without accidental navigation; Ellipsis remains decorative unless explicitly composed with a menu.
...
The default path uses items. Manual links need no root collection or value registration; Items, when used, belongs inside List.

```jsx
<>
  <Breadcrumb items={[{ label: 'Projects', href: '/projects' }, { label: 'Overview' }]} />

  <Breadcrumb>
    <Breadcrumb.List>
      <Breadcrumb.Item><Breadcrumb.Link href="/projects">Projects</Breadcrumb.Link></Breadcrumb.Item>
      <Breadcrumb.Separator />
      <Breadcrumb.Item><Breadcrumb.Page>Overview</Breadcrumb.Page></Breadcrumb.Item>
    </Breadcrumb.List>
  </Breadcrumb>
</>
```
...
`src/navigation/breadcrumb/breadcrumb.types.ts:7` anchors the current implementation contract:
...
The implementation entry is `src/navigation/breadcrumb/breadcrumb.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/navigation/breadcrumb`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `docs/pages/(navigation)/breadcrumb`
...
| --- | --- | --- |
| Focused regression | `nub run test src/navigation/breadcrumb` | Exit 0, matching tests executed and passing |
...
Use `codex/breadcrumb` if creating an isolated branch. Keep commits scoped, for example `refactor(breadcrumb): expose breadcrumb links without requiring duplicate data`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/navigation/breadcrumb` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test href/to priority, target/rel and current/disabled semantics, custom routing refs/events, separators, items-only versus manual markup, no duplicate List and SSR accessible navigation.
...
**Verify:** `nub run test src/navigation/breadcrumb` and `nub run typecheck` → exit 0, including the new acceptance cases.
