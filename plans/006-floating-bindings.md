# Plan 006: Separate floating geometry from surface animation
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/overlays/base test/browser`. Also inspect `git status --short` and `git diff -- src/overlays/base test/browser` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Keep useFloatingPosition responsible for reference/positioner geometry, middleware and observer cleanup, without Theme, cn, focus or dismiss dependencies. Preserve stale-Promise protection, virtual anchors, sameWidth, fitViewport and original-style restoration. Keep necessary size.apply DOM writes with explicit ownership. Put layout transform on a positioner and transition transform on the surface, adapting the current Popper facade without breaking public callers. Preserve inherited RTL and physical data-side.
...
Use `codex/floating-bindings` if creating an isolated branch. Keep commits scoped, for example `refactor(floating-bindings): separate floating geometry from surface animation`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
Cover out-of-order positioning promises, reference and floating node replacement, virtual pointer anchor, scroll ancestor changes, sameWidth/fitViewport, RTL, external styles and cleanup. In a browser assert both correct geometry and a working surface transform animation.
