# Plan 028: Expose numeric input controls without changing parsing contracts
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/forms/input-number 'docs/pages/(form)/input-number' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx docs/routes/components/markdown/docs-playground.tsx`. Also inspect `git status --short` and `git diff -- src/forms/input-number 'docs/pages/(form)/input-number' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx docs/routes/components/markdown/docs-playground.tsx` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Input/Increment/Decrement and keep the complete default control. Root owns raw/display state, minValue/maxValue/step and inputRef. rawValue retains precedence; onChange emits formatted string, onRawValueChange number. Parts inherit controls and bounds; increment/decrement default to type button with existing icons unless overridden. Preserve temporary editing text and hold-repeat behavior.
...
The complete default control stays available. Manual controls inherit numeric state and bounds; their default button types and icons do not need to be repeated.

```jsx
<>
  <InputNumber defaultValue={1} minValue={0} maxValue={10} />

  <InputNumber defaultValue={1} minValue={0} maxValue={10}>
    <InputNumber.Decrement aria-label="Decrease quantity" />
    <InputNumber.Input aria-label="Quantity" />
    <InputNumber.Increment aria-label="Increase quantity" />
  </InputNumber>
</>
```
...
The implementation entry is `src/forms/input-number/input-number.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/forms/input-number`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/forms/input-number` | Exit 0, matching tests executed and passing |
...
Use `codex/input-number` if creating an isolated branch. Keep commits scoped, for example `refactor(input-number): expose numeric input controls without changing parsing contracts`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/forms/input-number` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test incomplete negative/decimal text, formatting, rawValue precedence, bounds/step/precision, disabled/readOnly, repeat cancellation/unmount, submit/reset, inputRef and exact callback payloads.
...
**Verify:** `nub run test src/forms/input-number` and `nub run typecheck` → exit 0, including the new acceptance cases.
