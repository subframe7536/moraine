...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/shared/render-element.tsx src/shared/merge-element-props.ts src/shared/render-element.test.tsx src/shared/merge-element-props.test.tsx src/overlays/base/popper.tsx src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx AGENTS.md`. Also inspect `git status --short` and `git diff -- src/shared/render-element.tsx src/shared/merge-element-props.ts src/shared/render-element.test.tsx src/shared/merge-element-props.test.tsx src/overlays/base/popper.tsx src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx AGENTS.md` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Extract mergePopperElementProps into one private mergeElementProps implementation and add a small renderElement helper under src/shared. Keep renderComponentOrElement for content. Accept function-only host render; reject preconstructed JSX. Preserve lazy getters, Solid tuple event handlers, cancellable user-first actions, noncancellable disabled/lifecycle invariants, function ref composition and classList. Render exactly one subtree. Keep as and render mutually exclusive with useful currentTarget/ref types, without any or generic erasure. Add only the render naming exception to AGENTS.md. Migrate the existing Popper merger consumer; do not retrofit all components.
...
- `src/shared/render-element.test.tsx` (create if absent)
- `src/shared/merge-element-props.test.tsx` (create if absent)
- `src/overlays/base/popper.tsx`
...
| --- | --- | --- |
| Focused regression | `nub run test src/shared src/overlays/base` | Exit 0, matching tests executed and passing |
...
Use `codex/host-render` if creating an isolated branch. Keep commits scoped, for example `refactor(host-render): extract one host-render and dom-props merge protocol`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/shared src/overlays/base` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test user cancellation, disabled protection, tuple handlers, non-event function props, changing DOM props, refs on replacement and unmount, class/style priority and one invocation of the selected renderer. Add positive and negative declaration tests for as/render exclusivity and function-only render.
...
**Verify:** `nub run test src/shared src/overlays/base` and `nub run typecheck` → exit 0, including the new acceptance cases.
