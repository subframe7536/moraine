# Plan 010: Expose Tabs parts backed by one complete item collection
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/navigation/tabs 'docs/pages/(navigation)/tabs' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/theme-layout.test.tsx docs/routes/components/markdown/docs-code-tabs.tsx docs/routes/components/markdown/intro-components.tsx src/shared/use-selectable-collection-navigation.ts test/browser`. Also inspect `git status --short` and `git diff -- src/navigation/tabs 'docs/pages/(navigation)/tabs' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/theme-layout.test.tsx docs/routes/components/markdown/docs-code-tabs.tsx docs/routes/components/markdown/intro-components.tsx src/shared/use-selectable-collection-navigation.ts test/browser` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose List, Trigger, Content and Indicator, without Tabs.Items. Keep root items and the complete no-children widget. Manual root items require explicit stable value; root data owns order/disabled/default selection before evaluating JSX. Trigger/Content value reads label/content/disabled from root data. Omitted children inherits content, explicit empty stays empty. Default assembly reuses the same parts internally; never instantiate content twice or scan JSX to derive data. Mounted registrations hold only nodes, measurement and actual part existence. Preserve activationMode, onChange, RTL and indicator behavior.
...
Both paths use root items with lazy label/content values. The manual triggers and panels inherit their content and disabled state by value; there is no public Items assembler.
...
<>
  {/* Complete default widget. */}
  <Tabs items={tabs} />

  {/* Custom structure using the same data contract. */}
  <Tabs items={tabs}>
    <Tabs.List>
    ...
    <aside>Shared project context</aside>
...
`src/navigation/tabs/tabs.types.ts:6` anchors the current implementation contract:
...
The implementation entry is `src/navigation/tabs/tabs.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/navigation/tabs`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `docs/routes/components/markdown/docs-code-tabs.tsx`
...
| --- | --- | --- |
| Focused regression | `nub run test src/navigation/tabs` | Exit 0, matching tests executed and passing |
...
Use `codex/tabs` if creating an isolated branch. Keep commits scoped, for example `refactor(tabs): expose tabs parts backed by one complete item collection`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/navigation/tabs` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Cover Content-before-List in server HTML, no defaultValue, first item disabled, empty data, duplicate/unknown manual values, deletion/reordering, Show-hidden parts, custom dynamic IDs and non-dangling IDREFs. Test lazy inherited content, explicit empty override, keyboard activation/focus and indicator browser geometry. Re-run Stepper navigation regressions and compare Tabs consumer bundles.
...
**Verify:** `nub run test src/navigation/tabs` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
