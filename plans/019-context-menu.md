# Plan 019: Expose ContextMenu parts while retaining pointer anchors
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- test/browser src/overlays/context-menu 'docs/pages/(overlay)/context-menu' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/overlays/base`. Also inspect `git status --short` and `git diff -- test/browser src/overlays/context-menu 'docs/pages/(overlay)/context-menu' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/overlays/base` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Trigger/Portal/Positioner/Content/Panel/Group/GroupLabel/Item/Separator/Sub/SubTrigger/SubContent/Items. Move old Content items assembly to Panel. Panel may contain custom entries without forcing raw overlay anatomy; explicit children suppress automatic items even when temporarily empty. Items emits current-layer entries only. Sub owns exactly one logical child layer; SubContent is raw surface, while Panel inside Sub assembles its Portal/Positioner/surface. Preserve all existing item kinds and callbacks without adding unimplemented checked/radio products. Never share parent surface IDs/refs with children. Preserve right-click coordinates, virtual anchors and existing long-press/cancellation behavior. Reuse proven private menu pieces without making ContextMenu a button-anchored DropdownMenu.
...
The default Panel receives the existing items data. The manual alternative customizes entries while keeping Panel assembly. Each Sub owns a separate child layer; the nested Panel does not create a second child state. ContextMenu retains pointer-coordinate anchoring.

```jsx
<>
  <ContextMenu>
    <ContextMenu.Trigger>Right-click this area</ContextMenu.Trigger>
    <ContextMenu.Panel items={items} />
  </ContextMenu>
...
`src/overlays/context-menu/context-menu.types.ts:13` anchors the current implementation contract:
...
The implementation entry is `src/overlays/context-menu/context-menu.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/overlays/context-menu`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `docs/pages/(overlay)/context-menu`
...
| --- | --- | --- |
| Focused regression | `nub run test src/overlays/context-menu` | Exit 0, matching tests executed and passing |
...
Use `codex/context-menu` if creating an isolated branch. Keep commits scoped, for example `refactor(context-menu): expose contextmenu parts while retaining pointer anchors`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/overlays/context-menu` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test contextmenu coordinates, long-press cancellation, touch/mouse differences, keyboard opening, submenu ownership, dismissal/focus restore, scroll/RTL, custom items and real-browser virtual positioning.
...
**Verify:** `nub run test src/overlays/context-menu` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
