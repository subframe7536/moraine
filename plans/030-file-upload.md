# Plan 030: Expose file selection parts with original File identity
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/forms/file-upload 'docs/pages/(form)/file-upload' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx`. Also inspect `git status --short` and `git diff -- src/forms/file-upload 'docs/pages/(form)/file-upload' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose only Dropzone/Trigger/List/Item/Preview/Remove. Keep File|File[]|null and the mandatory native file input; this component selects/validates files and performs no HTTP upload. List defaults to complete rows. List.itemRender receives the original file inside Item context; explicit Item file={file} associates that original object, and Preview/Remove consume it. Use native markup for name/size instead of public Name/Size. Preserve rejection, object URL lifecycle and reset behavior.
...
The root retains the native file input. List owns Item wrappers; itemRender runs inside each Item context and receives the original File. Do not add another Item wrapper inside this renderer.
...
`src/forms/file-upload/file-upload.types.ts:24` anchors the current implementation contract:
...
The implementation entry is `src/forms/file-upload/file-upload.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/forms/file-upload`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/forms/file-upload` | Exit 0, matching tests executed and passing |
...
Use `codex/file-upload` if creating an isolated branch. Keep commits scoped, for example `refactor(file-upload): expose file selection parts with original file identity`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/forms/file-upload` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test native selection/drop, single/multiple/required/name, rejects and onFileReject, original File identity, custom row context, remove propagation, same-file reselection, reset and object URL cleanup on replace/remove/unmount.
...
**Verify:** `nub run test src/forms/file-upload` and `nub run typecheck` → exit 0, including the new acceptance cases.
