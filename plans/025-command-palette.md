# Plan 025: Expose CommandPalette layout backed by complete groups
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- test/browser src/navigation/command-palette 'docs/pages/(navigation)/command-palette' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/overlays/dialog/dialog.test.tsx docs/routes/components/layout/docs-command-palette.tsx src/overlays/base`. Also inspect `git status --short` and `git diff -- test/browser src/navigation/command-palette 'docs/pages/(navigation)/command-palette' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/overlays/dialog/dialog.test.tsx docs/routes/components/layout/docs-command-palette.tsx src/overlays/base` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
- **Effort**: L
- **Risk**: MED
- **Depends on**: [012-select-internals.md](012-select-internals.md), [003-host-render.md](003-host-render.md)
- **Category**: dx
...
Expose Input/List/Items/Item/Group/GroupLabel/Empty/Footer/Close only for a real rearranged layout. Keep root groups as complete data, searchTerm callbacks, item onSelect, inputRef/autofocus and virtual entries. Items emits inner rows only. Keep the component an embeddable panel; do not create a Dialog. Close calls the family callback, not the nearest arbitrary overlay. Reuse only genuinely shared navigation rules.
...
groups represents the complete command data supplied by the application. The manual layout stays embeddable; Close uses the existing CommandPalette close callback configured on the root.
...
<>
  ...
  <CommandPalette groups={groups} onClose={() => setPaletteOpen(false)}>
    ...
    <CommandPalette.List><CommandPalette.Items /></CommandPalette.List>
    <CommandPalette.Empty>No commands found</CommandPalette.Empty>
    <CommandPalette.Footer>
      <span>Choose a command</span>
      <CommandPalette.Close>Close</CommandPalette.Close>
    </CommandPalette.Footer>
  </CommandPalette>
</>
```
...
`src/navigation/command-palette/command-palette.types.ts:8` anchors the current implementation contract:
...
The implementation entry is `src/navigation/command-palette/command-palette.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/navigation/command-palette`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/navigation/command-palette` | Exit 0, matching tests executed and passing |
...
Use `codex/command-palette` if creating an isolated branch. Keep commits scoped, for example `refactor(command-palette): expose commandpalette layout backed by complete groups`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/navigation/command-palette` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test default/manual search and ranking, IME, disabled/group rows, item onSelect and close callbacks, virtual offscreen focus, absent Input, SSR, data changes and controlled query behavior.
...
**Verify:** `nub run test src/navigation/command-palette` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
