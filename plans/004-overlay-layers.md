# Plan 004: Separate overlay ancestry from activation order per document
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/overlays/base src/overlays/modal test/browser`. Also inspect `git status --short` and `git diff -- src/overlays/base src/overlays/modal test/browser` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
- **Effort**: M
- **Risk**: HIGH
- **Depends on**: [002-browser-regressions.md](002-browser-regressions.md)
- **Category**: tech-debt
...
Replace module-global layer ordering as an ancestry proxy with document-local registration and explicit logical parent IDs. Keep activation order separately for dismissal. Portal placement does not change ancestry. Share the storage where useful, but do not use one isTop boolean for dismissal, focus scope and outside hiding. Scope listeners and resource cleanup to the owning document. Guarantee two roots using one package instance; multi-copy coordination is deferred unless a real support requirement is established. Do not add EnvironmentProvider or a generic global registry.
...
Use `codex/overlay-layers` if creating an isolated branch. Keep commits scoped, for example `refactor(overlay-layers): separate overlay ancestry from activation order per document`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
Add failing regressions first for siblings mistaken for descendants, nested portaled layers, first Escape closing only the eligible layer, independent documents, same-document independent roots, unregister/reopen and listener release.
