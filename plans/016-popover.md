# Plan 016: Separate Popover positioning and custom content
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- test/browser src/overlays/popover 'docs/pages/(overlay)/popover' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/elements/button/button-group.test.tsx src/overlays/base/popper.test.tsx 'docs/pages/(overlay)/dialog/nested-overlays.tsx' 'docs/pages/(general)/button-group/button-popover.tsx'`. Also inspect `git status --short` and `git diff -- test/browser src/overlays/popover 'docs/pages/(overlay)/popover' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/elements/button/button-group.test.tsx src/overlays/base/popper.test.tsx 'docs/pages/(overlay)/dialog/nested-overlays.tsx' 'docs/pages/(general)/button-group/button-popover.tsx'` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Trigger/Anchor/Portal/Positioner/Content/Arrow/Close/Panel. Panel retains Portal+Positioner+Content and accepts arbitrary custom children; raw Content creates no assembly. Anchor changes the reference without creating another trigger. Preserve hover, modal/preventScroll and positioning options. Keep non-modal restore independent of trap.
...
Custom Panel children retain positioning assembly. Use the raw alternative only when the positioning/surface structure itself needs customization; it is a separate root, not a second surface in the first root.

```jsx
<>
  <Popover>
    <Popover.Trigger>Project options</Popover.Trigger>
    <Popover.Panel>
...
The implementation entry is `src/overlays/popover/popover.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/overlays/popover`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `src/overlays/base/popper.test.tsx`
- `docs/pages/(overlay)/dialog/nested-overlays.tsx`
...
| --- | --- | --- |
| Focused regression | `nub run test src/overlays/popover` | Exit 0, matching tests executed and passing |
...
Use `codex/popover` if creating an isolated branch. Keep commits scoped, for example `refactor(popover): separate popover positioning and custom content`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/overlays/popover` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test trigger versus anchor, custom panel heading, raw placement, click/hover timers, modal option, nested portals, focus restoration, RTL/virtual anchor, exit/reopen, no duplicate positioning and SSR IDs.
...
**Verify:** `nub run test src/overlays/popover` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
