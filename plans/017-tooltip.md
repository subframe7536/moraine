# Plan 017: Expose Tooltip positioning parts with descriptive semantics
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- test/browser src/overlays/tooltip 'docs/pages/(overlay)/tooltip' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/overlays/base/popper.test.tsx`. Also inspect `git status --short` and `git diff -- test/browser src/overlays/tooltip 'docs/pages/(overlay)/tooltip' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/overlays/base/popper.test.tsx` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Trigger/Portal/Positioner/Content/Arrow/Panel. Migrate old assembled Content to Panel, retaining text/kbds convenience there. Keep hover/focus delays and instant-open behavior. Tooltip is descriptive: no Close, Title, dialog role, focus trap or interactive surface controls. Share existing hover lifecycle without creating a new public interaction system.
...
The default path is Trigger plus Panel. The raw alternative remains descriptive and contains no close button or other interactive content.

```jsx
<>
  <Tooltip>
    <Tooltip.Trigger>Save</Tooltip.Trigger>
    <Tooltip.Panel>Save project changes</Tooltip.Panel>
  </Tooltip>
...
The implementation entry is `src/overlays/tooltip/tooltip.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/overlays/tooltip`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/overlays/tooltip` | Exit 0, matching tests executed and passing |
...
Use `codex/tooltip` if creating an isolated branch. Keep commits scoped, for example `refactor(tooltip): expose tooltip positioning parts with descriptive semantics`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/overlays/tooltip` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test hover/focus timing and cancellation, disabled trigger handling, aria-describedby for actual present content, custom IDs, default-open/forceMount/SSR, nested modal usage and cleanup.
...
**Verify:** `nub run test src/overlays/tooltip` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
