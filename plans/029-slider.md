# Plan 029: Expose Slider geometry parts with stable thumb indices
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/forms/slider 'docs/pages/(form)/slider' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/theme-layout.test.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx test/browser`. Also inspect `git status --short` and `git diff -- src/forms/slider 'docs/pages/(form)/slider' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/theme-layout.test.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx test/browser` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Track/Range/Thumb and keep default full rendering. Thumb index identifies a value slot, never a changing numeric value. Root owns number or number[] shape, orientation/range/minimum gaps/crossing and hidden form adaptation. Keep onValueChange for live updates and onChange for committed updates. Parts inherit state and required ARIA.
...
Thumb index identifies a stable slot in the root value array. Parts inherit range and orientation; the default root still assembles the full control.
...
The implementation entry is `src/forms/slider/slider.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/forms/slider`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/forms/slider` | Exit 0, matching tests executed and passing |
...
Use `codex/slider` if creating an isolated branch. Keep commits scoped, for example `refactor(slider): expose slider geometry parts with stable thumb indices`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/forms/slider` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test one/multiple thumbs, stable focus while values change, crossing/min-gap, keyboard/pointer/touch, vertical/RTL geometry, disabled/readOnly, live versus committed callbacks, submit/reset and generic value shape.
...
**Verify:** `nub run test src/forms/slider` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
