# Plan 014: Expose Modal assembly without adding another behavior kernel
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- test/browser src/overlays/modal 'docs/pages/(overlay)/modal' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/cn-context.test.tsx src/overlays/dialog/dialog.test.tsx src/overlays/dialog/dialog.tsx src/overlays/sheet/sheet.tsx src/overlays/base`. Also inspect `git status --short` and `git diff -- test/browser src/overlays/modal 'docs/pages/(overlay)/modal' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/cn-context.test.tsx src/overlays/dialog/dialog.test.tsx src/overlays/dialog/dialog.tsx src/overlays/sheet/sheet.tsx src/overlays/base` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Trigger/Portal/Overlay/Content/Close/Panel on the existing public Modal facade. Panel assembles only Portal/backdrop/surface; accessible name remains aria-label or the existing explicit association. Use Dialog for title/description/default body layout. Reuse private modal behavior and preserve current callers until this family is migrated.
...
Panel provides the default portal/backdrop/surface assembly. The raw alternative supplies those parts explicitly and names the surface without introducing Dialog layout behavior.

```jsx
<>
  <Modal>
    <Modal.Trigger>Open details</Modal.Trigger>
    <Modal.Panel aria-label="Project details">Project details</Modal.Panel>
  </Modal>
  ...
  <Modal>
    ...
    <Modal.Portal>
      <Modal.Overlay />
      <Modal.Content aria-label="Project details">
        <p>Project details</p>
...
The implementation entry is `src/overlays/modal/modal.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/overlays/modal`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/overlays/modal` | Exit 0, matching tests executed and passing |
...
Use `codex/modal` if creating an isolated branch. Keep commits scoped, for example `refactor(modal): expose modal assembly without adding another behavior kernel`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/overlays/modal` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test Panel/raw surface parity for controlled open, outside/Escape, focus restoration, nested locks, forceMount and SSR; verify explicit names and no duplicate portal or focus scope.
...
**Verify:** `nub run test src/overlays/modal` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
