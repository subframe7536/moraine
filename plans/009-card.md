# Plan 009: Make Card a manually assembled styled container
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/elements/card 'docs/pages/(general)/card' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx 'docs/pages/(general)/collapsible/composable.tsx' 'docs/pages/(general)/collapsible/uncontrolled.tsx'`. Also inspect `git status --short` and `git diff -- src/elements/card 'docs/pages/(general)/card' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx 'docs/pages/(general)/collapsible/composable.tsx' 'docs/pages/(general)/collapsible/uncontrolled.tsx'` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Header, Title, Description, Action, Content and Footer on callable Card. Card has only manual structure: no mode prop, Panel, Layout or implicit body. Remove header/description/action/footer convenience props and the visible-title convenience behavior; native title remains a standard tooltip attribute. Migrate every old title usage, including strings that still typecheck, into Card.Title. Root children are rendered unchanged. Preserve compact, native attributes and class/style/classes/styles. Content maps to existing body slot; classes.body stays valid. Title is a meaningful heading with as support. Use root style context and CSS/local layout for optional regions; do not register Footer/Action presence in business state.
...
Card has one manual structure. Card.Content supplies body spacing; native elements can sit directly under Card for full-width regions.

```jsx
<Card compact>
  <Card.Header>
    <Card.Title as="h2">Project settings</Card.Title>
    <Card.Description>Manage the project details.</Card.Description>
    <Card.Action><Button>Edit</Button></Card.Action>
  </Card.Header>
  <img src="/project-cover.jpg" alt="Project landscape" />
...
The implementation entry is `src/elements/card/card.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/elements/card`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `src/elements/card`
- `docs/pages/(general)/card`
- `src/shared/type-test/default/index.tsx`
- `src/shared/type-test/autocomplete/index.tsx`
- `docs/pages/(general)/collapsible/composable.tsx`
- `docs/pages/(general)/collapsible/uncontrolled.tsx`
...
| --- | --- | --- |
| Focused regression | `nub run test src/elements/card` | Exit 0, matching tests executed and passing |
...
Use `codex/card` if creating an isolated branch. Keep commits scoped, for example `refactor(card): make card a manually assembled styled container`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/elements/card` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test raw root children and full-width media have no implicit body; Content spacing and body slot overrides; missing/reordered Header/Action/Footer; compact and themes; visible title migration plus native tooltip title. Negative type cases reject composition and removed convenience props. Verify SSR first HTML and node reuse.
...
**Verify:** `nub run test src/elements/card` and `nub run typecheck` → exit 0, including the new acceptance cases.
