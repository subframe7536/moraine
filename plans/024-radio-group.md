# Plan 024: Add RadioGroup.Item with one selection authority
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/forms/radio-group 'docs/pages/(form)/radio-group' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/theme-layout.test.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx`. Also inspect `git status --short` and `git diff -- src/forms/radio-group 'docs/pages/(form)/radio-group' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/theme-layout.test.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Keep default option rendering and add Item value plus content without a second required root data array. Root owns single selection, name and form integration. Inherit required/readOnly/disabled and preserve keyboard roving/focus and callbacks. Do not introduce a new global Radio component.
...
Manual Item content inherits the root name, selection and form behavior without a second items declaration.

```jsx
<>
  <RadioGroup name="plan" items={['Personal', 'Team']} />

  <RadioGroup name="plan" defaultValue="personal">
    <RadioGroup.Item value="personal">
      <strong>Personal</strong><span>For individual projects</span>
    </RadioGroup.Item>
...
**Observed:** Native manual Items use one scalar authority and DOM-order keyboard navigation; disabled skipping, controlled rejection, cancelled reset and one submitted value pass.

**Production acceptance:** Prevent eager children evaluation and disconnected duplicate registrations. Prototype initial server tabIndex still depends on client registration; define an SSR-safe entry/selected tab stop and test it explicitly. IDREF/hydration tests do not cover this missing keyboard contract.
...
`src/forms/radio-group/radio-group.types.ts:12` anchors the current implementation contract:
...
The implementation entry is `src/forms/radio-group/radio-group.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/forms/radio-group`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/forms/radio-group` | Exit 0, matching tests executed and passing |
...
Use `codex/radio-group` if creating an isolated branch. Keep commits scoped, for example `refactor(radio-group): add radiogroup.item with one selection authority`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/forms/radio-group` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test arrows, disabled options, controlled selection, name/required/readOnly, native submit/reset, custom labels and conditional Items, SSR checked state and dynamic label IDs.
...
**Verify:** `nub run test src/forms/radio-group` and `nub run typecheck` → exit 0, including the new acceptance cases.
