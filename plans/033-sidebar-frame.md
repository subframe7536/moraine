# Plan 033: Expose SidebarFrame.Trigger through the existing toggle state
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/navigation/sidebar-frame 'docs/pages/(navigation)/sidebar-frame' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/cn-context.test.tsx src/shared/provider/theme-layout.test.tsx docs/routes/_app.tsx`. Also inspect `git status --short` and `git diff -- src/navigation/sidebar-frame 'docs/pages/(navigation)/sidebar-frame' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/cn-context.test.tsx src/shared/provider/theme-layout.test.tsx docs/routes/_app.tsx` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Add Trigger using the existing isOpen/toggle context with appropriate native type and ARIA. Keep Sidebar/SidebarHeader/SidebarBody/SidebarFooter/Main names and current desktop/mobile behavior. Do not create another menu state or redesign the root. Adapt Sheet calls only if its new API has already landed; do not require Sheet migration merely to add Trigger.
...
The existing regions keep their names. Trigger consumes the root toggle state, and SidebarFrame continues to own its responsive Sheet integration.

```jsx
<SidebarFrame>
  <SidebarFrame.Sidebar>
    <SidebarFrame.SidebarHeader>Workspace</SidebarFrame.SidebarHeader>
    <SidebarFrame.SidebarBody><nav aria-label="Workspace">Projects</nav></SidebarFrame.SidebarBody>
    <SidebarFrame.SidebarFooter>Account</SidebarFrame.SidebarFooter>
  </SidebarFrame.Sidebar>
  <SidebarFrame.Main>
    <SidebarFrame.Trigger>Toggle navigation</SidebarFrame.Trigger>
...
The implementation entry is `src/navigation/sidebar-frame/sidebar-frame.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/navigation/sidebar-frame`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `docs/pages/(navigation)/sidebar-frame`
...
| --- | --- | --- |
| Focused regression | `nub run test src/navigation/sidebar-frame` | Exit 0, matching tests executed and passing |
...
Use `codex/sidebar-frame` if creating an isolated branch. Keep commits scoped, for example `refactor(sidebar-frame): expose sidebarframe.trigger through the existing toggle state`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/navigation/sidebar-frame` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test keyboard/pointer toggling once, inherited open state, custom trigger accessible name, mobile Sheet close behavior, breakpoint transitions without duplicate content/form controls, SSR and retained region members.
...
**Verify:** `nub run test src/navigation/sidebar-frame` and `nub run typecheck` → exit 0, including the new acceptance cases.
