# Plan 018: Expose DropdownMenu parts and explicit submenu ownership
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- test/browser src/overlays/dropdown-menu 'docs/pages/(overlay)/dropdown-menu' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/elements/button/button-group.test.tsx src/elements/button/button-group.ssr.fixture.tsx 'docs/pages/(general)/button-group/button-dropdown.tsx' src/overlays/base`. Also inspect `git status --short` and `git diff -- test/browser src/overlays/dropdown-menu 'docs/pages/(overlay)/dropdown-menu' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/elements/button/button-group.test.tsx src/elements/button/button-group.ssr.fixture.tsx 'docs/pages/(general)/button-group/button-dropdown.tsx' src/overlays/base` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Trigger/Portal/Positioner/Content/Panel/Group/GroupLabel/Item/Separator/Sub/SubTrigger/SubContent/Items. Move old Content items assembly to Panel. Panel may contain custom entries without forcing raw overlay anatomy; explicit children suppress automatic items even when temporarily empty. Items emits current-layer entries only. Sub owns exactly one logical child layer; SubContent is raw surface, while Panel inside Sub assembles its Portal/Positioner/surface. Preserve all existing item kinds and callbacks without adding unimplemented checked/radio products. Never share parent surface IDs/refs with children.
...
The default Panel receives the existing items data. The manual alternative customizes entries while keeping Panel assembly. Each Sub owns a separate child layer; the nested Panel does not create a second child state. 

```jsx
<>
  <DropdownMenu>
    <DropdownMenu.Trigger>Open actions</DropdownMenu.Trigger>
    <DropdownMenu.Panel items={items} />
  </DropdownMenu>
...
`src/overlays/dropdown-menu/dropdown-menu.types.ts:13` anchors the current implementation contract:
...
The implementation entry is `src/overlays/dropdown-menu/dropdown-menu.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/overlays/dropdown-menu`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/overlays/dropdown-menu` | Exit 0, matching tests executed and passing |
...
Use `codex/dropdown-menu` if creating an isolated branch. Keep commits scoped, for example `refactor(dropdown-menu): expose dropdownmenu parts and explicit submenu ownership`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/overlays/dropdown-menu` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test keyboard arrows/Home/End/typeahead, disabled and existing item kinds, user cancellation, submenus/Escape, pointer transitions, custom Panel children and raw SubContent, no double entry generation, focus restoration, IDs and SSR.
...
**Verify:** `nub run test src/overlays/dropdown-menu` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
