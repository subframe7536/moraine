# Plan 027: Expose Avatar image and fallback with shared load state
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/elements/avatar 'docs/pages/(general)/avatar' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx 'docs/pages/(navigation)/sidebar-frame/header-footer-slots.tsx' 'docs/pages/(overlay)/popover/hover-mode.tsx' 'docs/pages/(overlay)/popover/trigger-modes.tsx'`. Also inspect `git status --short` and `git diff -- src/elements/avatar 'docs/pages/(general)/avatar' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx 'docs/pages/(navigation)/sidebar-frame/header-footer-slots.tsx' 'docs/pages/(overlay)/popover/hover-mode.tsx' 'docs/pages/(overlay)/popover/trigger-modes.tsx'` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Expose Image/Fallback while keeping the full default Avatar. Parts inherit root src/alt/text/fallback and load status; do not create competing requests or require duplicate source props. Preserve badge behavior and stale-request protection. AvatarGroup, AvatarGroupT and its recipe remain independent. Only change Kind when parts are actually exposed.
...
Image/Fallback inherit source, alternative text, fallback text and load state from Avatar. AvatarGroup remains an independent export.

```jsx
<>
  <Avatar src="/alex.jpg" alt="Alex" text="AL" />
  ...
  <AvatarGroup items={[{ text: 'AL' }, { text: 'JD' }]} />
</>
```
...
The implementation entry is `src/elements/avatar/avatar.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/elements/avatar`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `src/elements/avatar`
- `docs/pages/(general)/avatar`
- `src/shared/type-test/default/index.tsx`
- `src/shared/type-test/autocomplete/index.tsx`
- `docs/pages/(navigation)/sidebar-frame/header-footer-slots.tsx`
- `docs/pages/(overlay)/popover/hover-mode.tsx`
- `docs/pages/(overlay)/popover/trigger-modes.tsx`
...
| --- | --- | --- |
| Focused regression | `nub run test src/elements/avatar` | Exit 0, matching tests executed and passing |
...
Use `codex/avatar` if creating an isolated branch. Keep commits scoped, for example `refactor(avatar): expose avatar image and fallback with shared load state`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/elements/avatar` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test success/failure, src changes while loading, stale callbacks, text/icon fallback, omitted and explicit children, alt semantics, badge, SSR and AvatarGroup compatibility.
...
**Verify:** `nub run test src/elements/avatar` and `nub run typecheck` → exit 0, including the new acceptance cases.
