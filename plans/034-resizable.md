...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/elements/resizable 'docs/pages/(general)/resizable' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/theme-layout.test.tsx 'docs/pages/(navigation)/sidebar-frame/sheet-resizable-render.tsx' test/browser`. Also inspect `git status --short` and `git diff -- src/elements/resizable 'docs/pages/(general)/resizable' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/shared/provider/theme-layout.test.tsx 'docs/pages/(navigation)/sidebar-frame/sheet-resizable-render.tsx' test/browser` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Keep Panel/Handle, existing disable naming, resize/collapse action and Handle children state callback. Add shared host forwarding only where a demonstrated custom handle needs it. Preserve root-owned measurements, constraints, keyboard/pointer behavior and nested panel ownership; do not expose internal crossTarget or add wrapper parts.
...
Panel and Handle remain the canonical names. A custom host forwards the supplied props; the existing Handle children callback still receives handle state.
...
`src/elements/resizable/resizable.types.ts:8` anchors the current implementation contract:
...
The implementation entry is `src/elements/resizable/resizable.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/elements/resizable`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `docs/pages/(navigation)/sidebar-frame/sheet-resizable-render.tsx`
...
| --- | --- | --- |
| Focused regression | `nub run test src/elements/resizable` | Exit 0, matching tests executed and passing |
...
Use `codex/resizable` if creating an isolated branch. Keep commits scoped, for example `refactor(resizable): improve resizable host customization without renaming parts`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/elements/resizable` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test custom Handle forwarding and render-state callbacks, keyboard resize/collapse, pointer capture/cleanup, min/max and nested geometry, disabled state, lifecycle callback ordering and SSR.
...
**Verify:** `nub run test src/elements/resizable` and `nub run typecheck` → exit 0, including the new acceptance cases. Run `nub run test:browser` after the browser prerequisite has landed; expected exit 0 with actual browser assertions executed.
