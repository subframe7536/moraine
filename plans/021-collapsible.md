...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/elements/collapsible 'docs/pages/(general)/collapsible' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx`. Also inspect `git status --short` and `git diff -- src/elements/collapsible 'docs/pages/(general)/collapsible' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Keep existing Trigger/Content and controlled open/defaultOpen/onOpenChange. Add only Indicator if a concrete customization needs it; do not copy Accordion collections, Item or Items into a single collapsible. Reuse one presence owner and shared host render on applicable existing parts. Preserve disabled and unmount behavior.
...
Keep the existing single-root Trigger/Content anatomy. Indicator is intentionally omitted from this example because it remains conditional on a demonstrated need.

```jsx
<Collapsible>
  <Collapsible.Trigger render={(domProps) => <button {...domProps} />}>
...
`src/elements/collapsible/collapsible.types.ts:9` anchors the current implementation contract:
...
The implementation entry is `src/elements/collapsible/collapsible.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/elements/collapsible`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
| --- | --- | --- |
| Focused regression | `nub run test src/elements/collapsible` | Exit 0, matching tests executed and passing |
...
Use `codex/collapsible` if creating an isolated branch. Keep commits scoped, for example `refactor(collapsible): refine existing collapsible parts without duplicating state`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/elements/collapsible` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test host forwarding, controlled rejection, keyboard activation once, indicator state if introduced, conditional content, forceMount/current presence options, exit/reopen and actual-content aria-controls in SSR.
...
**Verify:** `nub run test src/elements/collapsible` and `nub run typecheck` → exit 0, including the new acceptance cases.
