# Plan 023: Add CheckboxGroup.Item with shared form state
...
>
> Drift check: `git diff --stat 7d9633ca405c2bcf256481298486c7daee3e1456..HEAD -- src/forms/checkbox-group 'docs/pages/(form)/checkbox-group' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx`. Also inspect `git status --short` and `git diff -- src/forms/checkbox-group 'docs/pages/(form)/checkbox-group' src/shared/type-test/default/index.tsx src/shared/type-test/autocomplete/index.tsx src/forms/form/form-field.test.tsx src/forms/shared/native-readonly.ssr.fixture.tsx` for uncommitted work. Expected prerequisite edits must be reconciled against the contract below and recorded before proceeding; unexplained drift is a STOP condition. Never overwrite existing user changes.
...
Keep the existing full items-driven default and add Item value plus custom label content. Manual children do not require a duplicate root item array. Root owns array selection and form adaptation; inherit name/required/readOnly/disabled and current value/callback contracts. Keep CheckboxGroup standalone and do not change Checkbox custom true/false values.
...
The default path accepts items. Manual Item content does not require a duplicate items array; native form behavior and selection belong to the group root.

```jsx
<>
  <CheckboxGroup name="teams" items={['Design', 'Engineering']} />

  <CheckboxGroup name="teams" defaultValue={['design']}>
    <CheckboxGroup.Item value="design">
      <strong>Design</strong><span>Product and visual design</span>
    </CheckboxGroup.Item>
    <CheckboxGroup.Item value="engineering">Engineering</CheckboxGroup.Item>
  </CheckboxGroup>
</>
```
...
The implementation entry is `src/forms/checkbox-group/checkbox-group.tsx`. The component implementation, types, classes, tests and SSR fixtures are colocated in `src/forms/checkbox-group`. The existing component tests are the test-structure exemplar; inspect them before adding regression cases.
...
- `docs/pages/(form)/checkbox-group`
...
| --- | --- | --- |
| Focused regression | `nub run test src/forms/checkbox-group` | Exit 0, matching tests executed and passing |
...
Use `codex/checkbox-group` if creating an isolated branch. Keep commits scoped, for example `refactor(checkbox-group): add checkboxgroup.item with shared form state`. Run the repository QA gate before any requested commit. Do not push, open a PR or publish without operator instruction.
...
**Verify:** `git status --short` and the drift command above → every existing change is attributed; all prerequisites are completed or their equivalent contracts verified. Run `nub run test src/forms/checkbox-group` → baseline passes, or pre-existing failures are recorded and the task is stopped before behavior changes.

### 2. Add the observable acceptance cases

Test default and custom Item submit/reset, inherited and item disabled state, required/name/readOnly, controlled rejection, complex label association, keyboard activation, SSR and array type inference.
...
**Verify:** `nub run test src/forms/checkbox-group` and `nub run typecheck` → exit 0, including the new acceptance cases.
